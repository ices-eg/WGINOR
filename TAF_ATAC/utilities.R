...
# If necessary install cmdstanr as follows:
if (requireNamespace("cmdstanr", quietly = TRUE) == FALSE) {
  install.packages("cmdstanr", repos = c("https://mc-stan.org/r-packages/", getOption("repos")))
  cmdstanr::install_cmdstan()
}
options(brms.backend = "cmdstanr") # use cmdstanr as backend for brms

preATAC <- function(data, year_start = NA, year_end_trend = NA, year_end_OL = NA, n_forecasts = 3, family = gaussian(), echo = TRUE) {
  # preliminary model building before running the main ATAC function
  ...
  prefits <- list(fit0, fit1, fit2)
  return(prefits)
}

ATAC <- function(prefits, data, year_start = NA, year_end_trend = NA, year_end_OL = NA, n_forecasts = 3, family = gaussian(), echo = TRUE) {
  ...
  if (duration >= 10) { # check that the series contains at least 10 observations
    # STEP #1: Trend model
    fit0 <- update(prefits[[1]], newdata = data2, sample_prior = "no", recompile = FALSE) # No AR process
    if (duration >= 20) { # if the series is long enough, and if it was possible to fit a spline model then try AR(1) and AR(2) models
      fit1 <- update(prefits[[2]], newdata = data2, sample_prior = "no", recompile = FALSE) # AR1 pprocess
      fit2 <- update(prefits[[3]], newdata = data2, sample_prior = "no", recompile = FALSE) # AR2 pprocess
    ...
    if (best_model == 1) {
      fit <- update(prefits[[1]], newdata = data3, sample_prior = "no", recompile = FALSE)
    } # No AR process
    if (best_model == 2) {
      fit <- update(prefits[[2]], newdata = data3, sample_prior = "no", recompile = FALSE)
    } # AR1 pprocess
    if (best_model == 3) {
      fit <- update(prefits[[3]], newdata = data3, sample_prior = "no", recompile = FALSE)
    } # AR2 pprocess
    model.OL <- fit
    ...
    trendOL <- OL.trend.brm %>%
      mutate(year = data4$x) %>%
      filter(year <= year_end_OL) %>%
      .$Estimate
