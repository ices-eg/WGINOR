...
for (ycol in c(2:dim(series)[2])) { # loop on all time-series
  #  for (ycol in c(4,8,9,14:23)){ # loop on Zoopk time-series
  inputseries <- series %>%
    transmute(x = Year, y = .[, ycol][[1]]) %>%
    filter(is.na(y) == 0) %>%
    as_tibble() # wrangle data prior to modelling
  ATAC.fit <- try(ATAC(inputseries, year_start = 1980, year_end_OL = 2022, n_forecasts = 3), TRUE) # fit models to an individual time series
