...
series <- read.taf(taf.data.path("NOSdata_2023.11.16.csv")) # read the data from the "standard" TAF data path (~/bootstrap/data/tree.csv)
colnames(series)[1] <- "Year" # force the first column to be named "Year"
info <- read.taf(taf.data.path("NOSdata_VariablesInfo_2023.11.16.csv")) # read the data from the "standard" TAF data path (~/bootstrap/data/tree.csv)
