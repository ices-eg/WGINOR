...
series <- read.taf(taf.data.path("NOSdata_2022.11.12.csv")) # read the data from the "standard" TAF data path (~/bootstrap/data/tree.csv)
colnames(series)[1] <- "Year" # force the first column to be named "Year"
info <- read.taf(taf.data.path("NOSdata_VariablesInfo_2022.11.12.csv")) # read the data from the "standard" TAF data path (~/bootstrap/data/tree.csv)
